...
class SaytimeScript
  ...
  def run
    ...
    output_file = tmp_file("current-time.#{SOUND_EXT}")
  ...
  # Try path, then other extension (gsm/ulaw), then base without wx/
  def sound_file_path(path)
    return path if File.exist?(path)
    other = path.sub(/\.(gsm|ulaw)$/i) { |m| m.downcase == '.gsm' ? '.ulaw' : '.gsm' }
    return other if path != other && File.exist?(other)
    if path.start_with?(BASE_SOUND_DIR + '/') || path.start_with?(BASE_SOUND_DIR) && path.include?('/wx/')
      base_path = path.sub('/wx/', '/')
      return base_path if File.exist?(base_path)
      base_other = base_path.sub(/\.(gsm|ulaw)$/i) { |m| m.downcase == '.gsm' ? '.ulaw' : '.gsm' }
      return base_other if base_path != base_other && File.exist?(base_other)
    end
    path
  end
...
files << add_sound_file(sound_file_path("#{sound_dir}/rpt/good#{greeting}.#{SOUND_EXT}"), @missing_files)
    end

    files << add_sound_file(sound_file_path("#{sound_dir}/rpt/thetimeis.#{SOUND_EXT}"), @missing_files)
    
    hour = now.hour
    minute = now.min
    
    if use_24hour
files << add_sound_file(sound_file_path("#{sound_dir}/digits/0.#{SOUND_EXT}"), @missing_files) if hour < 10
      files << format_number(hour, sound_dir)

      if minute == 0
        files << add_sound_file(sound_file_path("#{sound_dir}/digits/hundred.#{SOUND_EXT}"), @missing_files)
        files << add_sound_file(sound_file_path("#{sound_dir}/hours.#{SOUND_EXT}"), @missing_files)
      else
        files << add_sound_file(sound_file_path("#{sound_dir}/digits/0.#{SOUND_EXT}"), @missing_files) if minute < 10
        files << format_number(minute, sound_dir)
        files << add_sound_file(sound_file_path("#{sound_dir}/hours.#{SOUND_EXT}"), @missing_files)
      end
    else
      display_hour = (hour == 0 || hour == 12) ? 12 : (hour > 12 ? hour - 12 : hour)
files << add_sound_file(sound_file_path("#{sound_dir}/digits/#{display_hour}.#{SOUND_EXT}"), @missing_files)

      if minute != 0
        # Use "o" (oh) from letters for leading minute in 12-hour, e.g. 2:06 -> "2 o 6" not "2 zero 6"
        if minute < 10
          o_file = "#{sound_dir}/letters/o.#{SOUND_EXT}"
          p = sound_file_path(o_file)
          p = sound_file_path("#{sound_dir}/digits/0.#{SOUND_EXT}") unless File.exist?(p)
          files << add_sound_file(p, @missing_files)
        end
        files << format_number(minute, sound_dir)
      end
      am_pm = hour < 12 ? 'a-m' : 'p-m'
      files << add_sound_file(sound_file_path("#{sound_dir}/digits/#{am_pm}.#{SOUND_EXT}"), 0)
    end
  ...
  def process_weather(location_id)
    ...
    weather_condition_file_to_clean = tmp_file("condition.#{SOUND_EXT}")
    ...
    weather_condition_file = tmp_file("condition.#{SOUND_EXT}")
    sound_dir = @options[:custom_sound_dir] || BASE_SOUND_DIR
    ...
    if File.exist?(temp_file)
      ...
      required_files = [
        sound_file_path("#{sound_dir}/silence/1.#{SOUND_EXT}"),
        sound_file_path("#{sound_dir}/wx/weather.#{SOUND_EXT}"),
        sound_file_path("#{sound_dir}/wx/conditions.#{SOUND_EXT}"),
        weather_condition_file,
        sound_file_path("#{sound_dir}/wx/temperature.#{SOUND_EXT}"),
        sound_file_path("#{sound_dir}/wx/degrees.#{SOUND_EXT}")
      ]
      ...
      files = "#{required_files[0]} #{required_files[1]} #{required_files[2]} #{weather_condition_file} #{required_files[4]} "

      temp_value = temp.to_i
      if temp_value < 0
        files += add_sound_file(sound_file_path("#{sound_dir}/digits/minus.#{SOUND_EXT}"), 0)
        temp_value = temp_value.abs
      end

      files += format_number(temp_value, sound_dir)
      files += " #{required_files[5]} "
    else
...
return "#{sound_file_path("#{sound_dir}/digits/0.#{SOUND_EXT}")} " if abs_num == 0

    # Handle hundreds
    if abs_num >= 100
      hundreds = abs_num / 100
      files += "#{sound_file_path("#{sound_dir}/digits/#{hundreds}.#{SOUND_EXT}")} "
      files += "#{sound_file_path("#{sound_dir}/digits/hundred.#{SOUND_EXT}")} "
      abs_num %= 100
    ...
    if abs_num < 20
      files += "#{sound_file_path("#{sound_dir}/digits/#{abs_num}.#{SOUND_EXT}")} "
    else
      # Handle tens and ones
      tens = (abs_num / 10) * 10
      ones = abs_num % 10
      files += "#{sound_file_path("#{sound_dir}/digits/#{tens}.#{SOUND_EXT}")} "
      files += "#{sound_file_path("#{sound_dir}/digits/#{ones}.#{SOUND_EXT}")} " if ones > 0
    end
  ...
  # Convert a sound file to target format (gsm or ulaw) using sox when needed.
  # Returns path to use for reading: either original file or a temp converted file.
  # Temp files must be unlinked by caller when done.
  def convert_sound_to_format(file_path, target_ext)
    return file_path unless file_path =~ /\.(ulaw|gsm)$/i
    src_ext = Regexp.last_match(1).downcase
    return file_path if src_ext == target_ext.downcase

    return file_path unless File.executable?(SOX_BIN)
    sox = SOX_BIN
    temp_out = tmp_file("saytime_conv_#{Process.pid}_#{File.basename(file_path, '.*')}.#{target_ext}")

    success = if src_ext == 'ulaw'
                # ulaw: raw 8kHz 8-bit mu-law
                system(sox, '-t', 'raw', '-r', '8000', '-e', 'mu-law', '-b', '8', '-c', '1', file_path, '-t', 'gsm', temp_out, out: File::NULL, err: File::NULL)
              else
                # gsm -> ulaw
                system(sox, '-t', 'gsm', file_path, '-t', 'raw', '-r', '8000', '-e', 'mu-law', '-b', '8', '-c', '1', temp_out, out: File::NULL, err: File::NULL)
              end

    if success && File.exist?(temp_out)
      temp_out
    else
      File.unlink(temp_out) if File.exist?(temp_out)
      file_path
    end
  end
  ...
  def create_output_file(input_files, output_file)
    begin
      output_ext = File.extname(output_file).delete('.').downcase
      output_ext = 'gsm' if output_ext.empty? || (output_ext != 'ulaw' && output_ext != 'gsm')

      File.open(output_file, 'wb') do |out|
        files = input_files.split(/\s+/).select { |f| f =~ /\.(ulaw|gsm)$/ }
        files_processed = 0
        temp_files_to_clean = []
        ...
        files.each do |file|
          ...
          if File.exist?(file)
            path_to_read = convert_sound_to_format(file, output_ext)
            temp_files_to_clean << path_to_read if path_to_read != file && path_to_read.start_with?(TMP_DIR + '/')
            File.open(path_to_read, 'rb') do |in_file|
        ...
        temp_files_to_clean.each { |f| File.unlink(f) if File.exist?(f) }
  ...
  def play_announcement(node, asterisk_file)
    asterisk_file = asterisk_file.sub(/\.(ulaw|gsm)$/, '')
  ...
  def is_safe_path(file)
    ...
    return true if file.start_with?('/var/lib/asterisk/sounds')
