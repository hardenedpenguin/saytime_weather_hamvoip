...
SOUND_EXT = 'gsm'
COND_FILE = "/tmp/condition.#{SOUND_EXT}"
TIMEZONE_FILE = '/tmp/timezone'
# Weather/conditions live in base /var/lib/asterisk/sounds or in .../sounds/wx
VARLIB_SOUND_BASE = '/var/lib/asterisk/sounds'
VARLIB_WX_BASE = '/var/lib/asterisk/sounds/wx'
WEATHER_SOUND_DIR = [VARLIB_SOUND_BASE, VARLIB_WX_BASE].find { |d| Dir.exist?(d) } || VARLIB_SOUND_BASE
# Alternate wx dir when primary not found (e.g. sounds/en/wx vs sounds/wx vs base)
ALTERNATE_WX_BASE = case WEATHER_SOUND_DIR
  when VARLIB_SOUND_BASE then VARLIB_WX_BASE
  else File.join(File.dirname(VARLIB_WX_BASE), 'en', 'wx')
  end

CONFIG_PATH = '/etc/asterisk/local/weather.ini'

# Synonyms for condition sound lookup (e.g. systems that have fog.gsm but not foggy.gsm)
CONDITION_SYNONYMS = { 'foggy' => %w[fog mist], 'mist' => %w[fog foggy], 'fog' => %w[foggy mist] }.freeze
...
class WeatherScript
  ...
  def parse_options
    parser = OptionParser.new do |opts|
      ...
      opts.on('-k', '--insecure', 'Disable SSL certificate verification (for legacy systems)') do
  ...
  def show_usage
    ...
    puts "  -k, --insecure           Disable SSL certificate verification (legacy systems)"
  ...
  def load_config
    ...
    @config['ssl_verify'] ||= 'YES'
    # Allow env override for legacy systems (e.g. outdated SSL)
    @config['ssl_verify'] = (ENV['SAYTIME_SSL_VERIFY'].to_s =~ /^(0|no|false|off)$/i ? 'NO' : 'YES') if ENV.key?('SAYTIME_SSL_VERIFY')
    ...
    @config['ssl_verify'] = 'NO' if @options[:insecure]
  ...
  # Try path, then other extension (gsm/ulaw), then alternate base, then base without wx/
  def sound_file_path(path)
    ...
    if path.start_with?(WEATHER_SOUND_DIR + '/') || path == WEATHER_SOUND_DIR
      alt = path.sub(WEATHER_SOUND_DIR, ALTERNATE_WX_BASE)
      return alt if File.exist?(alt)
      alt_other = alt.sub(/\.(gsm|ulaw)$/i) { |m| m.downcase == '.gsm' ? '.ulaw' : '.gsm' }
      return alt_other if alt != alt_other && File.exist?(alt_other)
      if path.include?('/wx/')
  ...
  def process_weather_condition(condition_text)
    ...
    [condition_lower, condition_lower.gsub(/\s+/, '-'), condition_lower.gsub(/\s+/, '_'), condition_lower.gsub(/\s+/, '')].each do |variant|
      file = sound_file_path("#{WEATHER_SOUND_DIR}/#{variant}.#{SOUND_EXT}")
      if File.exist?(file)
    ...
    # Try synonyms (e.g. foggy -> fog, mist) when exact match not found
    if condition_files.empty? && CONDITION_SYNONYMS[condition_lower]
      CONDITION_SYNONYMS[condition_lower].each do |syn|
        file = sound_file_path("#{WEATHER_SOUND_DIR}/#{syn}.#{SOUND_EXT}")
        if File.exist?(file)
          condition_files << file
          break
        end
    ...
    if condition_files.empty?
      ...
      words.each do |word|
        file = sound_file_path("#{WEATHER_SOUND_DIR}/#{word}.#{SOUND_EXT}")
        if File.exist?(file)
          condition_files << file
        end
      end
    ...
    # Try pattern matching if still no match (gsm and ulaw; Hamvoip may have mixed)
    if condition_files.empty?
      words = condition_lower.split(/\s+/).reject(&:empty?)
      sorted_words = words.sort_by { |w| important_words.include?(w) ? 0 : (modifiers.include?(w) ? 1 : 2) }
      Dir.glob(["#{WEATHER_SOUND_DIR}/*.gsm", "#{WEATHER_SOUND_DIR}/*.ulaw", "#{ALTERNATE_WX_BASE}/*.gsm", "#{ALTERNATE_WX_BASE}/*.ulaw"]).each do |file|
        filename = File.basename(file, File.extname(file)).downcase
    ...
    if condition_files.empty?
      %w[clear sunny fair].find do |d|
        file = sound_file_path("#{WEATHER_SOUND_DIR}/#{d}.#{SOUND_EXT}")
        File.exist?(file) && condition_files << file
      end
    end
  ...
  def http_get(url, timeout = HTTP_TIMEOUT_SHORT, user_agent = nil, max_redirects = 5)
    ...
    begin
      ...
      if http.use_ssl? && @config['ssl_verify'].to_s.upcase == 'NO'
